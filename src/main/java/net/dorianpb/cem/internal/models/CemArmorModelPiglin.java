...
public class CemArmorModelPiglin<C extends CemModel, T extends LivingEntity> extends BipedEntityModel<T>{
	...
	private static <M extends CemModelPart> M prepPart(M modelpart, String name){
		...
		armorCandidates.forEach((armorPart, map) -> {
			if(armorCandidates.get(armorPart) != null){
				...
				switch(name){
					case EntityModelPartNames.HAT -> newarmor1.addCuboid(reverse? -8 : 1, 0, 0, 8, 8, 8, 0, false, false, 0, 0);
					case EntityModelPartNames.BODY -> newarmor1.addCuboid(reverse? -8 : 0, 0, 0, 8, 12, 4, 0, false, false, 16, 16);
					case EntityModelPartNames.RIGHT_ARM -> newarmor1.addCuboid(reverse? -4 : 0, 0, 0, 4, 12, 4, 0, false, false, 40, 16);
					case EntityModelPartNames.LEFT_ARM -> newarmor1.addCuboid(reverse? -4 : 0, 0, 0, 4, 12, 4, 0, true, false, 40, 16);
					case EntityModelPartNames.RIGHT_LEG -> newarmor1.addCuboid(reverse? -4 : 0, 0, 0, 4, 12, 4, 0, false, false, 0, 16);
					case EntityModelPartNames.LEFT_LEG -> newarmor1.addCuboid(reverse? -4 : 0, 0, 0, 4, 12, 4, 0, true, false, 0, 16);
				}
